...
document.addEventListener("DOMContentLoaded", function () {
    ...
    console.log("🚀 Engine Harga v9 (Antam Logam Mulia + Worker) Dimulai...");
    cekHargaHarian();
...
// ==========================================
// 2. ENGINE HARGA VIA CLOUDFLARE WORKER
// ==========================================
...
// Helper: Memanggil file _worker.js milik kita sendiri
...
// --- ENGINE EMAS (UPDATE KE LOGAM MULIA) ---
async function startGoldEngine(d) {
    ...
    // Target baru web resmi Antam
    const target = 'https://www.logammulia.com/id/harga-emas-hari-ini';

    try {
        const html = await fetchViaProxy(target, 'text');

        // Gunakan DOMParser untuk membaca struktur tabel Logam Mulia
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, "text/html");

        let foundPrice = 0;

        // Cari semua sel tabel (td)
        const tds = doc.querySelectorAll('td');
        for (let i = 0; i < tds.length; i++) {
            // Cari teks murni '1 gr'
            if (tds[i].textContent.trim().toLowerCase() === '1 gr') {
                // Harga Dasar Antam berada di <td> setelahnya (kolom ke-2)
        ...
        // Fallback keamanan jika DOM parser terlewat
        ...
        if (foundPrice > 500000) {
            console.log("✅ Harga Emas Antam Update:", foundPrice);
            ...
            throw new Error("Pola harga emas tidak ditemukan di tabel Logam Mulia");
        }

    } catch (e) {
        console.warn("⚠️ Gagal Emas:", e.message);
...
async function fetchStockPrice(index, ticker) {
    ...
    const apiUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${yahooTicker}?interval=1d&range=1d`;

    try {
        const data = await fetchViaProxy(apiUrl, 'json');
        ...
        if (data.chart && data.chart.result && data.chart.result.length > 0) {
            ...
            if (price && price > 0) {
                console.log(`✅ Saham ${ticker} Updated: ${price}`);
        ...
        console.error(`❌ Gagal Saham ${ticker}:`, e.message);
    }
    return false;
}
...
async function fetchSingleBibit(index, url, isPreview = false) {
    ...
    try {
        ...
        console.log("Gagal Bibit:", e.message);
...
// ==========================================
// SISA KODE (UI, LOGIC, GOOGLE DRIVE) TETAP SAMA
// ==========================================
...
function updateTampilan() { daftarAset.sort((a, b) => a.nama.localeCompare(b.nama)); let grandTotal = daftarAset.reduce((s, i) => s + (i.nilai > 0 ? i.nilai : 0), 0); document.getElementById('grandTotalDisplay').innerText = formatRupiah(grandTotal); simpanHistoryHarian(grandTotal); updateGoalUI(); updateRunwayUI(); renderRebalancingTable(); renderPieChart(); let sum = { 'reksa': 0, 'kas': 0, 'saham': 0, 'komo': 0, 'kripto': 0 }; let subSum = {}; daftarAset.forEach(a => { if (a.nilai <= 0 && !a.ticker) return; let k = dataKategori[a.kategori] ? a.kategori : 'reksa'; sum[k] += a.nilai; a.currentTrend = getAssetTrend(a.id, a.nilai); if (!subSum[a.subJenis]) subSum[a.subJenis] = { t: 0, w: 0 }; subSum[a.subJenis].t += a.nilai; subSum[a.subJenis].w += (a.currentTrend * a.nilai); }); for (let k in sum) { let el = document.getElementById(`sum-${k}`), ep = document.getElementById(`pct-${k}`); if (el) { el.innerText = formatRupiah(sum[k]); ep.innerText = grandTotal > 0 ? ((sum[k] / grandTotal) * 100).toFixed(1) + '%' : '0%'; } } let subH = ""; Object.keys(subSum).sort().forEach(k => { let d = subSum[k]; if (d.t <= 0) return; let avg = d.t > 0 ? d.w / d.t : 0; let vis = renderTrend(avg, d.t); subH += `<tr><td><b>${k}</b></td><td>${formatRupiah(d.t)}</td><td>${grandTotal > 0 ? ((d.t / grandTotal) * 100).toFixed(1) : 0}%</td><td>${vis.badge}</td></tr>`; }); document.getElementById('subCategoryTableBody').innerHTML = subH || "<tr><td colspan='4' style='text-align:center; color:#666;'>Tidak ada aset aktif</td></tr>"; let mastH = "", cA = 0; for (let i = 0; i < daftarAset.length; i++) { let a = daftarAset[i]; if (a.nilai <= 0 && (!a.ticker)) continue; cA++; let style = a.nilai <= 0 ? "opacity: 0.7" : ""; let vis = renderTrend(a.currentTrend, a.nilai); let det = a.subJenis; if (a.subJenis === 'Emas Batangan') det = `<span style="color:#ffca28">⚖️ ${a.berat.toFixed(2)}g</span>`; else if (a.url && a.url.includes('bibit.id')) det = `<span style="color:#00c853">🌱 ${a.berat.toFixed(2)} Unit</span>`; else if (a.ticker) det = `<span style="color:#29b6f6">📈 ${a.ticker} (${a.lot} Lot)</span>`; let kl = dataKategori[a.kategori] ? dataKategori[a.kategori].label : "Lain"; let priceInfo = ""; if (a.lastPrice && a.lastPrice > 0) { priceInfo = `<br><small style="color:#666; font-size:0.65rem;">Harga: ${new Intl.NumberFormat('id-ID').format(a.lastPrice)}</small>`; } mastH += `<tr style="${style}"><td><b>${a.nama}</b><br><small>${det}</small>${priceInfo}</td><td><small>${kl}</small></td><td><b>${formatRupiah(a.nilai)}</b></td><td>${grandTotal > 0 ? ((a.nilai / grandTotal) * 100).toFixed(1) : 0}%</td><td>${vis.badge}</td><td>${vis.nominal}</td><td class="action-cell"><button class="btn-mini-action btn-edit" onclick="siapkanEditAset(${i})">✏️</button><button class="btn-mini-action btn-delete" onclick="hapusAset(${i})">🗑️</button></td></tr>`; } document.getElementById('masterTableBody').innerHTML = cA === 0 ? "<tr><td colspan='7' style='text-align:center; padding:20px; color:#666;'>Dompet Kosong</td></tr>" : mastH; }
...
function simpanDataKeStorage() { localStorage.setItem('portfolio_assets_v1', JSON.stringify(daftarAset)); localStorage.setItem('local_last_updated', new DatetoISOString()); }
function loadDataAset() { const d = localStorage.getItem('portfolio_assets_v1'); if (d) { try { daftarAset = JSON.parse(d); let u = false; daftarAset.forEach(a => { if (!dataKategori[a.kategori]) { a.kategori = 'reksa'; u = true } }); if (u) simpanDataKeStorage(); updateTampilan(); } catch (e) { console.error("Data korup", e); } } }
...
async function uploadToDriveActual(silent) { if (!silent) showToast("Membackup data...", "info"); const syncTime = new Date().toISOString(); const content = JSON.stringify({ assets: localStorage.getItem('portfolio_assets_v1'), history: localStorage.getItem('portfolio_history'), last_synced: syncTime }); const fileMetadata = { 'name': G_FILENAME, 'mimeType': 'application/json' }; const fileBlob = new Blob([content], { type: 'application/json' }); try { const existingId = await findFileId(G_FILENAME); if (existingId) { await updateFileGoogle(existingId, fileBlob); } else { await createFileGoogle(fileMetadata, fileBlob); } if (!silent) showToast("Backup Berhasil!", "success"); else { const stat = document.getElementById('syncStatus'); if (stat) { stat.innerText = "TERSIMPAN"; setTimeout(() => { stat.innerText = "ONLINE"; }, 3000); } } } catch (err) { if (err.status === 401) { pendingAction = 'upload'; tokenClient.requestAccessToken({ prompt: '' }); } } }
async function restoreFromDriveActual(silent) { if (!silent) showToast("Mencari backup...", "info"); try { const fileId = await findFileId(G_FILENAME); if (!fileId) { if (!silent) showToast("Tidak ada backup.", "error"); return; } const res = await gapi.client.drive.files.get({ fileId: fileId, alt: 'media' }); const data = res.result; localStorage.setItem('portfolio_assets_v1', data.assets); localStorage.setItem('portfolio_history', data.history); if (!silent) showToast("Restore Berhasil!", "success"); setTimeout(() => location.reload(), 500); } catch (err) { if (err.status === 401) { pendingAction = 'restore'; tokenClient.requestAccessToken({ prompt: '' }); } } }
